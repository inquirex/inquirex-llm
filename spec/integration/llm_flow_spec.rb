...
# Realistic tax intake flow that mixes core verbs with the extract LLM verb.
# The LLM step is server-side only; the engine treats it as a collecting step
# whose answer is provided by the server adapter.
...
RSpec.describe "LLM flow integration" do
  ...
  describe "simple path (single filer)" do
    ...
    it "walks the short path straight to review" do
  ...
  describe "complex path (business filer)" do
    ...
    it "routes through business description and extract" do
  ...
  describe "JSON serialization" do
    ...
    it "serializes LLM metadata under 'llm' key" do
      extract_step = parsed["steps"]["business_extracted"]
      expect(extract_step["verb"]).to eq "extract"
      expect(extract_step["llm"]["prompt"]).to include("Extract structured")
      expect(extract_step["llm"]["schema"]).to eq(
        "industry"          => "string",
        "entity_type"       => "string",
        "employee_count"    => "integer",
        "estimated_revenue" => "currency"
      )
      expect(extract_step["llm"]["model"]).to eq "claude_sonnet"
      expect(extract_step["llm"]["temperature"]).to eq 0.2
    end
  ...
  describe "mixed node types" do
    ...
    it "all LLM nodes report collecting" do
      expect(LLM_TAX_FLOW.step(:business_extracted)).to be_collecting
